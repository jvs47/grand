...
class ConstantODEblock(ODEblock):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            elif self.opt.get('new_edges') == 'k_hop':
                self.add_khop_edges(k=2)
            elif self.opt.get('new_edges') == 'random_walk' and self.odefunc.attention_weights is not None:
                self.add_rw_edges()
        ...
        if self.opt.get("adjoint") and self.training:
            state_dt = integrator(
                ...
                options=dict(step_size=self.opt.get('step_size'), max_iters=self.opt.get('max_iters')),
                adjoint_method=self.opt.get('adjoint_method'),
                adjoint_options=dict(step_size=self.opt.get('adjoint_step_size'), max_iters=self.opt.get('max_iters')),
                atol=self.atol,
                rtol=self.rtol,

...
def train(model, optimizer, data, pos_encoding=None):
    ...
    if model.opt.get('use_labels'):
        train_label_idx, train_pred_idx = get_label_masks(data, model.opt.get('label_rate'))
    ...
    if model.opt.get('dataset') == 'ogbn-arxiv':
...
def main(cmd_opt):
    ...
    dataset = get_dataset(opt, f'{ROOT_DIR}/data', opt.get('not_lcc'))
    ...
    if opt.get('rewire_KNN') or opt.get('fa_layer'):
        model = GNN_KNN(opt, dataset, device).to(device) if opt.get("no_early") else GNNKNNEarly(opt, dataset,
                                                                                                 device).to(device)
    else:
        model = GNN(opt, dataset, device).to(device) if opt.get("no_early") else GNNEarly(opt, dataset, device).to(
            device)
    ...
    this_test = test_OGB if opt.get('dataset') == 'ogbn-arxiv' else test

    for epoch in range(1, opt.get('epoch')):
        start_time = time.time()

        if opt.get('rewire_KNN') and epoch % opt.get('rewire_KNN_epoch') == 0 and epoch != 0:
        ...
        if tmp_val_acc > val_acc:
            ...
            best_time = opt.get('time')
        if not opt.get('no_early') and model.odeblock.test_integrator.solver.best_val > val_acc:
            best_epoch = epoch

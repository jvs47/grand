...
def main(opt, run_count):
    ...
    for epoch in range(1, opt.get('epoch')):
        ...
        print(log.format(epoch, results['time'][-1], results['loss'][-1], results['forward_nfe'][-1], results[
            'backward_nfe'][-1], results['train_acc'][-1], results['val_acc'][-1], results['test_acc'][-1]))
        ...
        pickle.dump(results, open(
            f"../results/{opt.get('dataset')}_{opt.get('method')}_stepsize_{opt.get('dt')}_run_{run_count}.pickle",
            "wb"))
    ...
    if __name__ == '__main__':
        ...
        parser.add_argument('--alpha_dim', type=str, default='sc',
                            help='choose either scalar (sc) or vector (vc) alpha')
        ...
        parser.add_argument('--function', type=str, default='laplacian',
                            help='laplacian, transformer, dorsey, GAT, SDE')
        # ODE args
        ...
        opt['epoch'] = 31
        opt['adjoint'] = True
        # opt.get('method') = 'explicit_adams'
        opt['method'] = 'implicit_adams'
        # opt.get('method') = 'dopri5'
        opt['adjoint_method'] = opt.get('method')
        opt['max_iters'] = 10000
        ...
        for stepsize in [0.5, 0.25, 0.1, 0.01]:  # 2.0, 1.0
            ...
            for idx in range(opt.get('count_runs')):

...
def main(opt):
    ...
    dataset = get_dataset(opt, '../data', opt.get('not_lcc'))
    data = dataset.data

    device = torch.device(f"cuda:{opt.get('gpu')}" if torch.cuda.is_available() else 'cpu')

    model = Node2Vec(data.edge_index, embedding_dim=opt.get('embedding_dim'), walk_length=opt.get('walk_length'),
                     context_size=opt.get('context_size'), walks_per_node=opt.get('walks_per_node'),
                     num_negative_samples=opt.get('neg_pos_ratio'), p=1, q=1, sparse=True).to(device)
    ...
    ### here be main code
    t = time.time()
    for epoch in range(1, opt.get('epochs') + 1):
        loss = train()
        train_t = time.time() - t
        t = time.time()
        acc, _ = test()
    ...
    fname = "DW_%s_emb_%03d_wl_%03d_cs_%02d_wn_%02d_epochs_%03d.pickle" % (
        opt.get('dataset'), opt.get('embedding_dim'), opt.get('walk_length'), opt.get('context_size'),
        opt.get('walks_per_node'), opt.get('epochs')
    )

...
def create_regularization_fns(args):
    ...
    for arg_key, reg_fn in six.iteritems(REGULARIZATION_FNS):
        if args.get(arg_key, None) is not None:
...
class ODEblock(nn.Module):
    ...
    def set_tol(self):
        self.atol = self.opt.get('tol_scale') * 1e-7
        self.rtol = self.opt.get('tol_scale') * 1e-9
        if self.opt.get('adjoint'):
            self.atol_adjoint = self.opt.get('tol_scale_adjoint') * 1e-7
            self.rtol_adjoint = self.opt.get('tol_scale_adjoint') * 1e-9
...
class BaseGNN(MessagePassing):
    def __init__(self, opt, dataset, device=torch.device('cpu')):
        ...
        if opt.get('beltrami'):
            self.mx = nn.Linear(self.num_features, opt.get('feat_hidden_dim'))
            self.mp = nn.Linear(opt.get('pos_enc_dim'), opt.get('pos_enc_hidden_dim'))
            opt['hidden_dim'] = opt.get('feat_hidden_dim') + opt.get('pos_enc_hidden_dim')
        else:
        ...
        if opt.get('use_labels'):
            # todo - fastest way to propagate this everywhere, but error prone - refactor later
            opt['hidden_dim'] = opt.get('hidden_dim') + dataset.num_classes
        else:
            self.hidden_dim = opt.get('hidden_dim')
        if opt.get('fc_out'):
            self.fc = nn.Linear(opt.get('hidden_dim'), opt.get('hidden_dim'))
        self.m2 = nn.Linear(opt.get('hidden_dim'), dataset.num_classes)
        if self.opt.get('batch_norm'):
            self.bn_in = torch.nn.BatchNorm1d(opt.get('hidden_dim'))
            self.bn_out = torch.nn.BatchNorm1d(opt.get('hidden_dim'))

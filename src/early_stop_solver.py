...
class EarlyStopDopri5(RKAdaptiveStepsizeODESolver):
    ...
    def __init__(self, func, y0, rtol, atol, opt, **kwargs):
        ...
        self.best_val = 0
        self.best_test = 0
        self.max_test_steps = opt.get('max_test_steps')
        self.best_time = 0
        self.ode_test = self.test_OGB if opt.get('dataset') == 'ogbn-arxiv' else self.test
        self.dataset = opt.get('dataset')
        if opt.get('dataset') == 'ogbn-arxiv':
            self.lf = torch.nn.functional.nll_loss
            self.evaluator = Evaluator(name=opt.get('dataset'))
...
class EarlyStopInt(torch.nn.Module):
    def __init__(self, t, opt, device=None):
        ...
        self.m2_bias = None
        self.opt = opt
        self.t = torch.tensor([0, opt.get('earlystopxT') * t], dtype=torch.float).to(self.device)

...
from run_GNN import add_labels
...
class ODEFunc(nn.Module):
    ...
    def __init__(self, in_features, out_features, opt, adj):
        ...
        self.w = nn.Parameter(torch.eye(opt.get('hidden_dim')))
        self.d = nn.Parameter(torch.zeros(opt.get('hidden_dim')) + 1)
...
class CGNN(nn.Module):
    def __init__(self, opt, adj, time, device):
        ...
        self.bm = Meter()
        self.device = device
        self.m1 = nn.Linear(opt.get('num_feature'), opt.get('hidden_dim'))

        self.odeblock = ODEblock(ODEFunc(2 * opt.get('hidden_dim'), 2 * opt.get('hidden_dim'), opt, adj),
                                 t=torch.tensor([0, self.T]))

        self.m2 = nn.Linear(opt.get('hidden_dim'), opt.get('num_class'))
...
def get_sym_adj(data, opt, device):
    edge_index, edge_weight = gcn_norm_fill_val(data.edge_index, data.edge_attr, opt.get('self_loop_weight'),
                                                data.num_nodes,
                                                dtype=data.x.dtype)
...
def main(opt):
    ...
    model, data = CGNN(opt, adj, opt.get('time'), device).to(device), dataset.data.to(device)
    print(opt)
    ...
    for epoch in range(1, opt.get('epoch')):
...
def train_ray_icml(opt, checkpoint_dir=None, data_dir="../data", opt_val=False):
    ...
    if opt.get("num_splits") > 0:
        dataset.data = set_train_val_test_split(
            23 * np.random.randint(0, opt.get("num_splits")),
            dataset.data,
            num_development=5000 if opt.get("dataset") == "CoauthorCS" else 1500)
...
def run_best_params(opt):
    ...
    if opt.get('dataset') == 'Cora':
        best_params = get_cora_opt(opt)
    elif opt.get('dataset') == 'Citeseer':
        best_params = get_citeseer_opt(opt)
    elif opt.get('dataset') == 'Pubmed':
    ...
    result = tune.run(
        ...
        name=opt.get('name'),
        resources_per_trial={"cpu": opt.get('cpus'), "gpu": opt.get('gpus')},
        config=best_params,
        num_samples=opt.get('num_splits'),
        scheduler=None,
        max_failures=3,
        local_dir='../ray_tune',

...
def find_or_make_encodings(opt):
    ...
    fname = os.path.join(POS_ENC_PATH, f"{opt.get('dataset')}_{opt.get('pos_enc_type')}.pkl")
...
def main(opt):
    ...
    dim = opt.get('embedding_dim')
    type = opt.get('pos_enc_type')
    model = NMF(n_components=dim, init='random', random_state=0, max_iter=opt.get('max_iter'), verbose=1,
                tol=opt.get('tol'))
    fname = os.path.join(POS_ENC_PATH, f"{opt.get('dataset')}_{opt.get('pos_enc_type')}.pkl")
    print(f"[i] Looking for positional encodings in {fname}...")
    ...
    out_path = f"{opt.get('out_dir')}/compressed_pos_encodings_{dim}_{type}.pkl"
    with opt(out_path, 'wb') as f:
        pickle.dump(W, f)

    if not os.path.exists(opt.get('out_path')):
        os.makedirs(opt.get('out_path'))
    with open(fname, "wb") as f:
        pickle.dump(W, f)

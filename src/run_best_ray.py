...
def get_best_params_dir(opt):
    analysis = ExperimentAnalysis(f"../ray_tune/{opt.get('folder')}")
    df = analysis.dataframe(metric=opt.get('metric'), mode='max')
    best_params_dir = df.sort_values('accuracy', ascending=False)['logdir'].iloc[opt.get('index')]
...
def run_best_params(opt):
    ...
    best_params_ret['epoch'] = opt.get('epoch')
    best_params_ret['max_nfe'] = opt.get('max_nfe')
    # handle adjoint
    if best_params['adjoint'] or opt.get('adjoint'):
    ...
    result = tune.run(
        ...
        num_samples=opt.get('reps') if opt.get("num_splits") == 0 else opt.get("num_splits") * opt.get("reps"),
        scheduler=None,
    ...
    df = result.dataframe(metric=opt.get('metric'), mode="max").sort_values(opt.get('metric'), ascending=False)
    try:

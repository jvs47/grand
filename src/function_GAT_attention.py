...
class ODEFuncAtt(ODEFunc):
    ...
    def __init__(self, in_features, out_features, opt, data, device):
        ...
        assert self.attention_dim % opt.get('heads') == 0, "Number of heads must be a factor of the dimension size"
        self.d_k = self.attention_dim // opt.get('heads')
...
class SpGraphAttentionLayer(nn.Module):
    ...
    def forward(self, x, edge):
        ...
        attention = softmax(edge_e, edge[self.opt.get('attention_norm_idx')])
        return attention, wx

...
from post_analysis_image import print_image_T, plot_att_heat, print_image_path
...
def get_image_opt(opt):
    ...
    opt['num_nodes'] = opt.get('im_height') * opt.get('im_width')  # * opt['im_chan'] #2708  ###pixels
...
def train(epoch, model, optimizer, dataset):
    model.train()
    loader = DataLoader(dataset, batch_size=model.opt.get('batch_size'), shuffle=True)

    for batch_idx, batch in enumerate(loader):
        optimizer.zero_grad()
        start_time = time.time()

        if batch_idx > model.opt.get('train_size') // model.opt.get('batch_size'):  # only do this for 1st batch/epoch
            break
...
def test(model, dataset):
    ...
    batch_size = model.opt.get('batch_size')
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
    total_correct = 0
    for batch_idx, batch in enumerate(loader):
        if batch_idx > model.opt.get('test_size') // model.opt.get('batch_size'):  # only do this for 1st batch/epoch
            break
        model.eval()
...
def main(opt):
    ...
    model = GNN_image(opt, batch, opt.get('num_class'), device, is_debug=True).to(device)
    ...
    optimizer = get_optimizer(opt.get('optimizer'), parameters, lr=opt.get('lr'), weight_decay=opt.get('decay'))
    best_val_acc = test_acc = best_epoch = 0

    for epoch in range(opt.get('epoch')):  # 1, opt.get('epoch')):
    ...
    # out = model(batch.x)
    # model.eval()
    # print_image_T(model, Graph_test, opt, modelpath=None, height=opt.get('im_height'), width=opt.get('im_width'))
    # animation = print_image_path(model, Graph_test, opt, height=2, width=1, frames=10)
    # animation.save(f'../images/Graph{exdataset}_ani_{timestr}.gif', writer='imagemagick',

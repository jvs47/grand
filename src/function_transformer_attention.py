...
class SpGraphTransAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, opt, device, concat=True, edge_weights=None):
        ...
        self.alpha = opt.get('leaky_relu_slope')
        self.concat = concat
        self.device = device
        self.opt = opt
        self.h = int(opt.get('heads'))
        self.edge_weights = edge_weights

        try:
            self.attention_dim = opt.get('attention_dim')
        except KeyError:
        ...
        if self.opt.get('beltrami') and self.opt.get('attention_type') == "exp_kernel":
            ...
            self.Qx = nn.Linear(opt.get('hidden_dim') - opt.get('pos_enc_hidden_dim'), self.attention_dim)
            self.init_weights(self.Qx)
            self.Vx = nn.Linear(opt.get('hidden_dim') - opt.get('pos_enc_hidden_dim'), self.attention_dim)
            self.init_weights(self.Vx)
            self.Kx = nn.Linear(opt.get('hidden_dim') - opt.get('pos_enc_hidden_dim'), self.attention_dim)
            self.init_weights(self.Kx)

            self.Qp = nn.Linear(opt.get('pos_enc_hidden_dim'), self.attention_dim)
            self.init_weights(self.Qp)
            self.Vp = nn.Linear(opt.get('pos_enc_hidden_dim'), self.attention_dim)
            self.init_weights(self.Vp)
            self.Kp = nn.Linear(opt.get('pos_enc_hidden_dim'), self.attention_dim)
            self.init_weights(self.Kp)

        else:
            if self.opt.get('attention_type') == "exp_kernel":
    ...
    def forward(self, x, edge):
        ...
        # if self.opt.get('beltrami') and self.opt.get('attention_type') == "exp_kernel":
        if self.opt.get('beltrami') and self.opt.get('attention_type') == "exp_kernel":
            label_index = self.opt.get('feat_hidden_dim') + self.opt.get('pos_enc_hidden_dim')
            p = x[:, self.opt.get('feat_hidden_dim'): label_index]
            x = torch.cat((x[:, :self.opt.get('feat_hidden_dim')], x[:, label_index:]), dim=1)
            ...
            dst_k = k[edge[1, :], :, :]
        prods = None
        if not self.opt.get('beltrami') and self.opt.get('attention_type') == "exp_kernel":
            prods = self.output_var ** 2 * torch.exp(
                -(torch.sum((src - dst_k) ** 2, dim=1) / (2 * self.lengthscale ** 2)))
        elif self.opt.get('attention_type') == "scaled_dot":
            prods = torch.sum(src * dst_k, dim=1) / np.sqrt(self.d_k)
        elif self.opt.get('attention_type') == "cosine_sim":
            cos = torch.nn.CosineSimilarity(dim=1, eps=1e-5)
            prods = cos(src, dst_k)
        elif self.opt.get('attention_type') == "pearson":
        ...
        if self.opt.get('reweight_attention') and self.edge_weights is not None:
            prods = prods * self.edge_weights.unsqueeze(dim=1)
        if self.opt.get('square_plus'):
            attention = squareplus(prods, edge[self.opt.get('attention_norm_idx')])
        else:
            attention = softmax(prods, edge[self.opt.get('attention_norm_idx')])
...
if __name__ == '__main__':
    ...
    opt = {'dataset': 'Cora', 'self_loop_weight': 1, 'leaky_relu_slope': 0.2, 'heads': 2, 'K': 10,
           'attention_norm_idx': 0, 'add_source': False, 'attention_dim': 30, 'attention_type': 'scaled_dot',
           'alpha_dim': 'sc', 'beta_dim': 'sc', 'max_nfe': 1000, 'mix_features': False, 'square_plus': True
           }

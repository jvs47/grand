...
class RewireAttODEblock(ODEblock):
    def __init__(self, odefunc, regularization_fns, opt, data, device, t=torch.tensor([0, 1]), gamma=0.5):
        ...
        assert opt.get('att_samp_pct') > 0 and opt.get(
            'att_samp_pct') <= 1, "attention sampling threshold must be in (0,1]"
        self.opt = opt
        self.odefunc = odefunc(self.aug_dim * opt.get('hidden_dim'), self.aug_dim * opt.get('hidden_dim'), opt, data,
                               device)
        ...
        if opt.get('function') not in {'GAT', 'transformer'}:
            self.multihead_att_layer = SpGraphTransAttentionLayer(opt.get('hidden_dim'), opt.get('hidden_dim'), opt,
                                                                  device, edge_weights=self.odefunc.edge_weight).to(
                device)

    def get_attention_weights(self, x):
        if self.opt.get('function') not in {'GAT', 'transformer'}:
    ...
    def renormalise_attention(self, attention):
        index = self.odefunc.edge_index[self.opt.get('attention_norm_idx')]
    ...
    def add_random_edges(self):
        # M = self.opt.get("M_nodes")
        # M = int(self.num_nodes * (1/(1 - (1 - self.opt.get('att_samp_pct'))) - 1))
        M = int(self.num_nodes * (1 / (1 - (self.opt.get('rw_addD'))) - 1))
    ...
    def add_khop_edges(self, k=2, rm_self_loops=True):
        ...
        # if self.opt.get('threshold_type') == 'topk_adj':
        ...
        #
        # elif self.opt.get('threshold_type') == 'addD_rvR':
        #   pass
        #   # AN = A2
        ...
        #   #
        #   # threshold = torch.quantile(AN.values()[added_mask], 1 - self.opt.get('rw_addD'))
    ...
    def densify_edges(self):
        if self.opt.get('new_edges') == 'random':
            self.add_random_edges()
        elif self.opt.get('new_edges') == 'random_walk':
            self.add_rw_edges()
        elif self.opt.get('new_edges') == 'k_hop_lap':
            pass
        elif self.opt.get('new_edges') == 'k_hop_att':
            self.add_khop_edges(k=2)

    def threshold_edges(self, x, threshold):
        # get mean attention
        # i) sparsify on S_hat
        if self.opt.get('new_edges') == 'k_hop_att' and self.opt.get('sparsify') == 'S_hat':
            attention_weights = self.odefunc.attention_weights
            mean_att = attention_weights
        # ii) sparsify on recalculated attentions
        else:  # elif self.opt.get('sparsify') == 'recalc_att':
            attention_weights = self.get_attention_weights(x)
            mean_att = attention_weights.mean(dim=1, keepdim=False)

        if self.opt.get('use_flux'):
        ...
        # threshold = torch.quantile(mean_att, 1 - self.opt.get('att_samp_pct'))
    ...
    def forward(self, x):
        ...
        if self.training:
            with torch.no_grad():
                ...
                threshold = torch.quantile(self.odefunc.edge_weight, 1 / (pc_change - self.opt.get('rw_addD')))
        ...
        if self.opt.get("adjoint") and self.training:
            state_dt = integrator(
                ...
                options={'step_size': self.opt.get('step_size')},
                adjoint_method=self.opt.get('adjoint_method'),
                adjoint_options={'step_size': self.opt.get('adjoint_step_size')},
                atol=self.atol,
                rtol=self.rtol,

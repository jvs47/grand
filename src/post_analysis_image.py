...
from run_GNN import merge_cmd_args


@torch.no_grad()
def print_image_T(model, dataset, opt, modelpath, height=2, width=3):
    loader = DataLoader(dataset, batch_size=opt.get('batch_size'), shuffle=True)
    ...
    for i in range(height * width):
        ...
        if opt.get('im_dataset') == 'MNIST':
            plt.imshow(batch.x[torch.nonzero(mask)].view(model.opt.get('im_height'), model.opt.get('im_width')),
                       cmap='gray', interpolation='none')
        elif opt.get('im_dataset') == 'CIFAR':
            A = batch.x[torch.nonzero(mask)].view(model.opt.get('im_height'), model.opt.get('im_width'),
                                                  model.opt.get('im_chan'))
            A = A / 2 + 0.5
            plt.imshow(A)
        ...
        plt.axis('off')
        if opt.get('im_dataset') == 'MNIST':
            plt.imshow(out[i, :].view(model.opt.get('im_height'), model.opt.get('im_width')), cmap='gray',
                       interpolation='none')
        elif opt.get('im_dataset') == 'CIFAR':
            A = out[i, :].view(model.opt.get('im_height'), model.opt.get('im_width'), model.opt.get('im_chan'))
            A = A / 2 + 0.5
            plt.imshow(A)
...
def print_image_path(model, dataset, opt, height, width, frames):
    ...
    for i in range(height * width):
        ...
        mask = batch.batch == i
        if opt.get('im_dataset') == 'MNIST':
            plt.imshow(paths[i, 0, :].view(model.opt.get('im_height'), model.opt.get('im_width')), cmap='gray',
                       interpolation='none')
        elif opt.get('im_dataset') == 'CIFAR':
            A = paths[i, 0, :].view(model.opt.get('im_height'), model.opt.get('im_width'), model.opt.get('im_chan'))
            A = A / 2 + 0.5
            plt.imshow(A)
        plt.title("t=0 Ground Truth: {}".format(batch.y[i].item()))
        plt.axis('off')
    ...
    def update(ii):
        for i in range(height * width):
            ...
            plt.tight_layout()
            if opt.get('im_dataset') == 'MNIST':
                plt.imshow(paths[i, ii, :].view(model.opt.get('im_height'), model.opt.get('im_width')), cmap='gray',
                           interpolation='none')
            elif opt.get('im_dataset') == 'CIFAR':
                A = paths[i, ii, :].view(model.opt.get('im_height'), model.opt.get('im_width'),
                                         model.opt.get('im_chan'))
...
@torch.no_grad()
def plot_att_heat(model, model_key=None, modelpath=None):
    im_height = model.opt.get('im_height')
    im_width = model.opt.get('im_width')
    im_chan = model.opt.get('im_chan')
    hwc = im_height * im_width * im_chan
    slice = torch.tensor(range(hwc + 1))
    edge_index = model.odeblock.odefunc.edge_index
    num_nodes = model.opt.get('num_nodes')
    ...
    square_att = dense_att.view(model.opt.get('num_nodes'), model.opt.get('num_nodes'))
...
def main(cmd_opt):
    ...
    # loader = DataLoader(Graph_train, batch_size=model.opt.get('batch_size'), shuffle=True)
    loader = DataLoader(Graph_train, batch_size=opt.get('batch_size'), shuffle=True)
    for batch_idx, batch in enumerate(loader):
    ...
    # model = GNN_image(opt, batch, opt.get('num_class'), device).to(device)
    ...
    # 1)
    print_image_T(model, Graph_test, opt, height=2, width=2)  # width=3)
    # 2)

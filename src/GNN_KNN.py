...
from graph_rewiring import add_edges, edge_sampling
from model_configurations import set_block, set_function
from utils import get_full_adjacency
...
class GNN_KNN(BaseGNN):
    ...
    def forward(self, x, pos_encoding):
        ...
        if self.opt.get('use_labels'):
            y = x[:, -self.num_classes:]
            x = x[:, :-self.num_classes]

        if self.opt.get('beltrami'):
            x = F.dropout(x, self.opt.get('input_dropout'), training=self.training)
            x = self.mx(x)
            if self.opt.get('dataset') == 'ogbn-arxiv':
                p = pos_encoding
            else:
                p = F.dropout(pos_encoding, self.opt.get('input_dropout'), training=self.training)
                p = self.mp(p)
        ...
        if self.opt.get('use_mlp'):
            ...
            x = F.dropout(x + self.m11(F.relu(x)), self.opt.get('dropout'), training=self.training)
            x = F.dropout(x + self.m12(F.relu(x)), self.opt.get('dropout'), training=self.training)
        ...
        if self.opt.get('fa_layer'):
            temp_time = self.opt.get('time')
            temp_method = self.opt.get('method')
            temp_step_size = self.opt.get('step_size')
            ...
            if self.opt.get('edge_sampling_rmv') != 0:

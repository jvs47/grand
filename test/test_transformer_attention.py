...
class AttentionTests(unittest.TestCase):
    ...
    def test_function(self):
        ...
        self.assertTrue(attention.shape == (self.edge.shape[1], self.opt.get('heads')))
        ...
        def get_round_sum(tens, n_digits=3):
            val = torch.sum(tens, dim=int(not self.opt.get('attention_norm_idx')))
        ...
        self.assertTrue(attention.shape == (data.edge_index.shape[1], self.opt.get('heads')))
    ...
    def test_head_aggregation(self):
        ...
        ax1 = torch.mean(torch.stack(
            [torch_sparse.spmm(self.edge, attention[:, idx], self.x.shape[0], self.x.shape[0], self.x) for idx in
             range(self.opt.get('heads'))], dim=0), dim=0)
